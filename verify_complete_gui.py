...
def run_gui():
    ...
    root = tk.Tk()
    root.title("Hallucination Checker")
    ...
    def select_xlsx():
        ...
        if len(files) > 7:
            messagebox.showerror("오류", "엑셀 파일은 최대 7개까지 가능합니다.")
            return
        xlsx_paths.clear()
        xlsx_paths.extend(files)
        messagebox.showinfo("선택 완료", f"{len(files)}개 파일 선택됨")

    # -------------------
    # 실행 버튼
    # -------------------
    def run_verify():
        if not result_path.get() or not pdf_path.get():
            messagebox.showwarning("경고", "result.txt와 source.pdf를 선택하세요.")
            return

        out_dir = Path(result_path.get()).parent / "verify_output"
        out_dir.mkdir(exist_ok=True)

        try:
            out = verify(
                Path(result_path.get()),
                Path(pdf_path.get()),
                out_dir,
                xlsx_paths=[Path(p) for p in xlsx_paths] if xlsx_paths else None,
            )
    ...
    title = tk.Label(root, text="Hallucination Checker", font=("Arial", 16, "bold"))
    title.pack(pady=20)

    tk.Button(root, text="result.txt 선택", width=30, command=select_result).pack(pady=5)
    tk.Label(root, textvariable=result_path, wraplength=450).pack()

    tk.Button(root, text="source.pdf 선택", width=30, command=select_pdf).pack(pady=5)
    tk.Label(root, textvariable=pdf_path, wraplength=450).pack()

    tk.Button(root, text="xlsx 파일 선택 (최대 7개)", width=30, command=select_xlsx).pack(pady=10)

    tk.Button(root, text="검증 실행", width=25, height=2, bg="#2e7d32", fg="white",
              command=run_verify).pack(pady=20)
